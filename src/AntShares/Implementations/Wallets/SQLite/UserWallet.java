...
public class UserWallet extends Wallet
{
    ...
    @Override
    public void addContract(AntShares.Wallets.Contract contract)
    {
    ...
    @Override
    public AntShares.Wallets.Account createAccount(byte[] privateKey)
    {
    	AntShares.Wallets.Account account = super.createAccount(privateKey);
        onCreateAccount(account);
        addContract(AntShares.Wallets.Contract.createSignatureContract(account.publicKey));
        return account;
    }
    ...
    @Override
    public AntShares.Wallets.Coin[] findUnspentCoins(UInt256 asset_id, Fixed8 amount)
    {
    	AntShares.Wallets.Coin[] coins = findUnspentCoins(Arrays.stream(findUnspentCoins()).filter(p -> getContract(p.scriptHash).isStandard()), asset_id, amount);
...
//    }
    ...
    @Override
    protected AntShares.Wallets.Account[] loadAccounts()
    {
        try (WalletDataContext ctx = new WalletDataContext(dbPath()))
        {
        	Account[] entities = ctx.getAccounts();
        	AntShares.Wallets.Account[] accounts = new AntShares.Wallets.Account[entities.length];
        	for (int i = 0; i < accounts.length; i++)
        	{
        		byte[] decryptedPrivateKey = decryptPrivateKey(entities[i].privateKeyEncrypted);
        		accounts[i] = new AntShares.Wallets.Account(decryptedPrivateKey);
    ...
    @Override
    protected AntShares.Wallets.Coin[] loadCoins()
    {
        try (WalletDataContext ctx = new WalletDataContext(dbPath()))
        {
        	Coin[] entities = ctx.getCoins();
        	AntShares.Wallets.Coin[] coins = new AntShares.Wallets.Coin[entities.length];
        	for (int i = 0; i < coins.length; i++)
        	{
        		coins[i] = new AntShares.Wallets.Coin();
    ...
    @Override
    protected AntShares.Wallets.Contract[] loadContracts()
    {
        try (WalletDataContext ctx = new WalletDataContext(dbPath()))
        {
        	Contract[] entities = ctx.getContracts();
        	AntShares.Wallets.Contract[] contracts = new AntShares.Wallets.Contract[entities.length];
        	for (int i = 0; i < contracts.length; i++)
        	{
        		contracts[i] = Serializable.from(entities[i].rawData, AntShares.Wallets.Contract.class);
        	}
        	return contracts;
        }
...
//    }
    ...
    private void onCoinsChanged(WalletDataContext ctx, AntShares.Wallets.Coin[] added, AntShares.Wallets.Coin[] changed, AntShares.Wallets.Coin[] deleted)
    {
    ...
    @Override
    protected void onProcessNewBlock(Block block, AntShares.Wallets.Coin[] added, AntShares.Wallets.Coin[] changed, AntShares.Wallets.Coin[] deleted)
    {
    	ArrayList<AntShares.Core.Transaction> tx_changed = new ArrayList<AntShares.Core.Transaction>();
        try (WalletDataContext ctx = new WalletDataContext(dbPath()))
        {
        	ctx.beginTransaction();
            for (AntShares.Core.Transaction tx : block.transactions)
            {
...
//            TransactionsChanged?.Invoke(this, GetTransactionInfo(tx_changed));
    ...
    @Override
    protected void onSaveTransaction(AntShares.Core.Transaction tx, AntShares.Wallets.Coin[] added, AntShares.Wallets.Coin[] changed)
    {
        ...
        try (WalletDataContext ctx = new WalletDataContext(dbPath()))
        {
            ...
            onCoinsChanged(ctx, added, changed, new AntShares.Wallets.Coin[0]);
            ctx.commit();
        }
        //TODO
